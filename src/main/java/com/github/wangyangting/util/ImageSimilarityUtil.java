 ...
 * 
 */
public final class ImageSimilarityUtil {
 
    public static float compareSimilarity(String sourcePath, String destPath) {
        float ret = 0;
        try {
            ImageSimilarityUtil fp1 = new ImageSimilarityUtil(ImageIO.read(new File(sourcePath)));
            ImageSimilarityUtil fp2 = new ImageSimilarityUtil(ImageIO.read(new File(destPath)));
            ret = fp1.compare(fp2);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static float compareSimilarity(BufferedImage sourceImage, BufferedImage destImage) {
        float ret = 0;
        ImageSimilarityUtil fp1 = new ImageSimilarityUtil(sourceImage);
        ImageSimilarityUtil fp2 = new ImageSimilarityUtil(destImage);
        ret = fp1.compare(fp2);
        return ret;
    }
    ...
    public ImageSimilarityUtil(byte[] hashValue) {
    ...
    public ImageSimilarityUtil(String hashValue) {
        this(toBytes(hashValue));
    }
 
    public ImageSimilarityUtil(BufferedImage src) {
        this(hashValue(src));
    }
    ...
    /**
     * 从压缩格式指纹创建{@link ImageSimilarityUtil}对象
     * 
     * @param compactValue
     * @return
     */
    public static ImageSimilarityUtil createFromCompact(byte[] compactValue) {
        return new ImageSimilarityUtil(uncompact(compactValue));
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof ImageSimilarityUtil) {
            return Arrays.equals(this.binaryzationMatrix,
                    ((ImageSimilarityUtil) obj).binaryzationMatrix);
        } else {
     ...
     * @param compactValue
     * @return
     * @see #compare(ImageSimilarityUtil)
     */
    ...
    public float compare(String hashValue) {
        return compare(new ImageSimilarityUtil(hashValue));
    }
    ...
    public float compare(BufferedImage image2) {
        return compare(new ImageSimilarityUtil(image2));
    }
     ...
     */
    public float compare(ImageSimilarityUtil src) {
    ...
    public static float compare(BufferedImage image1, BufferedImage image2) {
        return new ImageSimilarityUtil(image1).compare(new ImageSimilarityUtil(image2));
    }
}
